...
interface EventMap
  extends VideoPresentationEvents,
 ...
 */
function transformNativeEvent<K extends keyof EventMap>(
  type: K,
  event: any
): EventMap[K] {
  switch (type) {
    case 'ConferenceStatusUpdatedEvent':
      return {
        ...event,
        status: ConferenceStatus[<ConferenceStatus>event.status],
      };
    case 'PermissionRefusedEvent':
      return {
        ...event,
        permission:
          PermissionRefusedType[<PermissionRefusedType>event.permission],
      };
    default:
      return event; //no other transformation from native's string to JS's enum values
  }
}
...
export default class IAPIEvents {
  ...
  public addListener<K extends keyof EventMap>(
    ...
    const callback = (event: EventMap[K]) =>
      listener(transformNativeEvent(type, event));

    return IAPIRawEvent.addListener(type, callback);
  }
}
