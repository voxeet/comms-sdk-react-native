...
function toMap(mediaDevice: MediaDevice): any {
  ...
  const platformConnectionState = fromConnectionState(
    mediaDevice.platformConnectionState
  );
...
const toMediaDevice = (map: any): MediaDevice => {
  ...
  const platformConnectionState = toConnectionState(
    map.platformConnectionState
  );
...
export default class AudioService extends AbstractService {
  ...
  public async registerUpdateDevices(
